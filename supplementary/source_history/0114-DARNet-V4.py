...
# 运行示例：python supplementary/source_history/0114-DARNet-V4.py --mode patch --seed 42
# %% md
# - [x] 实现
    ...
    for epoch in range(1, number_epoch + 1):
        ...
        for i, (inputs, labels) in loop:
            ...
            if ENABLE_PATCH:
                ...
                last_input = inputs[:, -1:, :original_feature_dim]  # 取前original_feature_dim个特征，与labels维度匹配
            ...
            if i % 5 == 0:
                ...
                for inp, lab in valid_loader:
                    ...
                    if ENABLE_PATCH:
                        ...
                        last_input = inp[:, -1:, :original_feature_dim]  # 取前original_feature_dim个特征，与lab维度匹配
    ...
    with torch.no_grad():
        ...
        for inputs, labels in test_loader:
            ...
            if patch_size > 1:
                ...
                decoder_step = inputs[:, -1:, :original_feature_dim].clone()
            else:
...
def run_training(patch_size=1, save_path="./checkpoints/model_state_dict.pt",
                 results_path="./results/results.csv", use_attention=False, use_seasonality=False):
...
if __name__ == "__main__":
    ...
    if args.mode == 'baseline':
        ...
        run_training(patch_size=4, use_attention=True, save_path="./checkpoints/patch_attention_state_dict.pt",
                     results_path="./results/patch_attention.csv")
        ...
        run_training(patch_size=1, use_attention=False, use_seasonality=True, 
                     ...
                     results_path="./results/season.csv")
        ...
        run_training(patch_size=4, use_attention=True, use_seasonality=True,
                     ...
                     results_path="./results/full_model.csv")
    elif args.mode == 'all':
        print("=== 运行所有消融实验 ===")
