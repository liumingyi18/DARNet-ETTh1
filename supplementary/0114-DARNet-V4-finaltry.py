...
# 运行示例：python supplementary/0114-DARNet-V4-finaltry.py --mode patch --seed 42
# %% md
# - [x] 实现
...
def build_optimizer(parameters, optimizer_name, lr, weight_decay):
...
def build_scheduler(optimizer, scheduler_name, number_epoch, scheduler_gamma=0.98):
    scheduler_name = scheduler_name.lower()
    if scheduler_name == "step":
        return torch.optim.lr_scheduler.StepLR(optimizer, 1, gamma=scheduler_gamma)
    if scheduler_name == "cosine":
        return torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=max(number_epoch, 1),
...
def load_result_metrics(results_path):
    records = pd.read_csv(results_path)
    return records.iloc[0].to_dict()


def append_optimization_log(log_path, row_dict):
...
# ## Fourier 季节性特征
# %%
def add_fourier_features(data, variant='basic'):
    """
    在原始数据上新增紧凑的日/周周期特征。
    ...
    if variant == 'basic':
        ...
        raise ValueError(f"Unsupported seasonality variant: {variant}")

    for period, harmonic, name in harmonics:
        data_with_fourier[f'sin_{name}'] = np.sin(2 * np.pi * harmonic * t / period)
        data_with_fourier[f'cos_{name}'] = np.cos(2 * np.pi * harmonic * t / period)
...
# ## load data
# %%
def load_etth1_data(use_seasonality=False, seasonality_variant='basic'):
    """加载ETTh1数据并预处理"""
    url = './data/ETTh1.csv'
    ...
    if use_seasonality:
        data = add_fourier_features(data, variant=seasonality_variant)
        print(f"=== 季节性特征已启用 ({seasonality_variant})，数据维度: {data.shape} ===")
...
class TimeAwareAdditiveAttention(nn.Module):
    """Additive attention with a learned relative-time bias."""

    def __init__(self, key_size, query_size, num_hiddens, dropout,
                 time_period=24.0, **kwargs):
        super(TimeAwareAdditiveAttention, self).__init__(**kwargs)
        self.W_k = nn.Linear(key_size, num_hiddens, bias=False)
        self.W_q = nn.Linear(query_size, num_hiddens, bias=False)
        self.w_v = nn.Linear(num_hiddens, 1, bias=False)
        self.time_mlp = nn.Sequential(
            nn.Linear(4, num_hiddens, bias=False),
            nn.Tanh(),
    ...
    def build_time_bias(self, query_len, key_len, device, dtype):
        query_positions = torch.arange(query_len, device=device, dtype=dtype).view(query_len, 1)
        key_positions = torch.arange(key_len, device=device, dtype=dtype).view(1, key_len)
        relative_positions = key_positions - query_positions
        recency = (key_len - 1) - key_positions

        normalizer = max(float(key_len - 1), 1.0)
        relative_norm = relative_positions / normalizer
        recency_norm = recency / normalizer
        periodic_sin = torch.sin(2 * math.pi * recency / self.time_period)
        periodic_cos = torch.cos(2 * math.pi * recency / self.time_period)

        time_features = torch.stack(
            (relative_norm, recency_norm, periodic_sin, periodic_cos),
            dim=-1
        ).unsqueeze(0)
        return self.time_mlp(time_features).squeeze(-1)
    ...
    def forward(self, queries, keys, values):
        ...
        time_bias = self.build_time_bias(
            queries.shape[1],
            keys.shape[1],
            queries.device,
            queries.dtype
...
class RNN_Attn_Block(nn.Module):
    def __init__(self, input_size, hidden_dim, i, dropout=0.5, use_attention=True):
        ...
        self.RNN = nn.GRU(input_size=rnn_input_size,
                          ...
                          batch_first=True,
                          dropout=dropout)
        # attention1 attends over encoder timesteps, so make it explicitly time-aware.
        # attention2 attends over the RNN-CNN feature map and stays unchanged.
        self.attention1 = TimeAwareAdditiveAttention(hidden_dim, hidden_dim, hidden_dim, dropout)
...
def train_model(train_x, train_y, valid_x, valid_y, input_size, seq_len,
                ...
                use_attention=False, target_index=6, grad_clip_norm=1.0, warmup_epochs=5,
                loss_name="mse", optimizer_name="adam", scheduler_name="step", scheduler_gamma=0.98,
                eval_interval=5):
    # 根据patch_size进行数据切片
    if patch_size > 1:
    ...
    optimizer = build_optimizer(model.parameters(), optimizer_name, lr, weight_decay)
    scheduler = build_scheduler(optimizer, scheduler_name, number_epoch, scheduler_gamma=scheduler_gamma)
    ...
    for epoch in range(1, number_epoch + 1):
        ...
        for i, (inputs, labels) in loop:
            ...
            # eval
            should_eval = (i % eval_interval == 0) or (i == len(train_loader) - 1)
            if should_eval:
                valid_losses = list()
                model.eval()
                for inp, lab in valid_loader:
...
def run_training(patch_size=1, save_path="./checkpoints/model_state_dict.pt",
                 ...
                 use_seasonality=False, seed=42, batch_size=256, lr=1e-3,
                 number_epoch=40, hidden_dim=64, n_layers=2, drop_prob=0,
                 weight_decay=0, grad_clip_norm=1.0, warmup_epochs=5,
                 loss_name="mse", optimizer_name="adam", scheduler_name="step",
                 scheduler_gamma=0.98, seasonality_variant='basic', eval_interval=5):
    ...
    print(f"  seasonality_variant: {seasonality_variant}")
    ...
    model, train_loss_list, valid_loss_list = train_model(
        ...
        batch_size=batch_size, lr=lr, drop_prob=drop_prob, weight_decay=weight_decay,
        save_path=save_path,
        patch_size=patch_size,
        use_attention=use_attention,
...
def run_logged_experiment(round_id, experiment_name, mode_or_strategy,
                          new_feature_or_change, train_setting_notes,
                          log_path, baseline_rmse, **config):
    run_training(**config)
    metrics = load_result_metrics(config['results_path'])
    row = {
        'round_id': round_id,
        'experiment_name': experiment_name,
        'mode_or_strategy': mode_or_strategy,
        'new_feature_or_change': new_feature_or_change,
        'patch_size': config.get('patch_size', 1),
        'use_attention': config.get('use_attention', False),
        'use_seasonality': config.get('use_seasonality', False),
        'batch_size': config.get('batch_size', 256),
        'hidden_dim': config.get('hidden_dim', 64),
        'n_layers': config.get('n_layers', 2),
        'lr': config.get('lr', 1e-3),
        'drop_prob': config.get('drop_prob', 0),
        'weight_decay': config.get('weight_decay', 0),
        'epochs': config.get('number_epoch', 40),
        'train_setting_notes': train_setting_notes,
        'final_train_loss': metrics.get('final_train_loss'),
        'final_valid_loss': metrics.get('final_valid_loss'),
        'MAE': metrics.get('MAE'),
        'RMSE': metrics.get('RMSE'),
        'RRSE': metrics.get('RRSE'),
        'CORR': metrics.get('CORR'),
        'baseline_RMSE': baseline_rmse,
        'better_than_baseline': metrics.get('RMSE') < baseline_rmse
    }
    append_optimization_log(log_path, row)
    return metrics, row
