...
# 运行示例：python code/run_all_modes.py --mode patch --seed 42
# %% md
# - [x] 实现
...
def add_fourier_features(data):
    ...
    data_with_fourier['sin_day'] = np.sin(2 * np.pi * t / 24)
    data_with_fourier['cos_day'] = np.cos(2 * np.pi * t / 24)
    data_with_fourier['sin_week'] = np.sin(2 * np.pi * t / 168)
    data_with_fourier['cos_week'] = np.cos(2 * np.pi * t / 168)
